...
@section('breadcrumb')
    <ol class="breadcrumb border-0 m-0">
        ...
        <li class="breadcrumb-item"><a href="{{ route('sales.index') }}">{{ __('sidebar.sales') }}</a></li>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="payment_method" id="payment_method" required>
                                                @foreach (App\Interface\PaymentMethod::getAllPaymentMethod() as $item)
                                                    <option {{ $salePayment->payment_method == $item->displayName() ? 'selected' : '' }} value="{{ $item->displayName() }}">{{ $item->displayName() }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
